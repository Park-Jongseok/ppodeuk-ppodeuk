...
class TaskService {
  /// [TaskService] 생성자
  TaskService({TaskRepository? taskRepository, ScoreService? scoreService})
  ...
  Future<void> _validateTaskPoints({
    ...
    // 할 일 수정 시, 기존 할 일의 점수를 openPoints에서 제외
  ...
  Future<int> updateTask(int id, Map<String, dynamic> taskData) async {
    ...
    if (oldTask == null) {
      throw Exception('수정할 할 일을 찾을 수 없습니다.');
    }
  ...
  Future<int> deleteTask(int id) async {
    ...
    // 공간 점수 업데이트 (삭제된 할 일의 공간 ID를 알아야 하지만,
    // 현재 구조에서는 공간 ID를 알 수 없으므로 전체 공간 점수를 업데이트해야 함)
    // TODO: 할 일 삭제 시 해당 공간의 점수만 업데이트하도록 개선 필요
